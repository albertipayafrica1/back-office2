...
const CreateAccountForm = ({ countryCode, rc, emailAlertHandler }) => {
  ...
  const updateRouteOnReferralChange = () => {
    ...
    return router.push(
      `/auth/createAccount?country=${formValues.countryOfOperation}&rc=${formValues.referral}`,
      undefined,
      { shallow: true }
    );
  };
      ...
      axios(config)
        .then((response) => {
          ...
          if (response.data.success === true) {
            ...
            setTimeout(() => {
              router.replace(
                `/auth/login?country=${formValues.countryOfOperation}&status=newAccountCreated`
              );
            }, 3000);
          } else {
            setLoading(false);
    ...
    <>
      <Stack sx={styles.topContainer} spacing={1}>
        ...
        <Stack direction="row" alignItems="center" spacing={3}>
          ...
          <Link href={`/auth/login?country=${query.country}`}>
